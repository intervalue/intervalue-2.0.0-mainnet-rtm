...
public class Event {
     ...
     * last -表示本event可以see到的本分片各个creator的最大seq值，默认为-1，当存在parent时，值为两个parent的last每个creator对应分量中lastSeq的最大值
     */
    Event[] lastEvents;
    Event[] firstEvents;
    int[] lastSeqs;
    ...
    Event(int shardId, long creatorId, long creatorSeq, long otherId, long otherSeq,
          Event selfParent, Event otherParent, Instant timeCreated,
          byte[] signature, long generation, byte[] hash, byte[][] transactions) {
