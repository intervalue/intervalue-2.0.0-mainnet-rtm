...
public class Hashnet {
    ...
    /**
     * 由外部调用，说明到给定round已经可以删除了
     * hashnet内部还会判断，确保没有未达成共识Event才能删除
     * @param round
     */
    ...
    private void consSetAncestorFirstEvent(Event event) {
        if (!event.isCleared()) {
            ...
            for (int i = 0; i < event.lastEvents.length; ++i) {
                for (Event x = event.lastEvents[i];
                     x != null && x.firstEvents != null && x.firstEvents[myId] == null;
                     x = x.getSelfParent()) {
    ...
    private RoundInfo consSetRoundCreated(Event event, ArrayList<Event> stronglySeen) {
        int numMembers = this.numNodes;
        long selfParentRound =
                event.getSelfParent() == null ? 0L : event.getSelfParent().roundCreated;
        long otherParentRound = event.getOtherParent() == null
                ? 0L
                : event.getOtherParent().roundCreated;
    ...
    /**
     * 使用指定event对其所有可以stronglySeen的witness进行投票
     * 即更新election内的vote值
     * @param event
     * @param roundInfo
     * @param stronglySeen
     */
    private void consVote(Event event, RoundInfo roundInfo,
                          ArrayList<Event> stronglySeen) {
        //自己必须是witness才可以投票
        if (event.isWitness) {
            int myId = (int) event.getCreatorId();

            //更新round内所有elections指定event对应的creatorid的值
            for (RoundInfo.ElectionRound e = roundInfo.elections; e != null;
                 e = e.nextElection) {
                ...
                if (e.age == 1L) {
                    ...
                    boolean superMajority =
                            (double) yesCount > 2.0D * (double) numMembers / 3.0D ||
                    ...
                    if (e.age % (long) Config.coinFreq == 0L) {
                        ...
                        if (!superMajority) {
                            if (e.age % (long) (2 * Config.coinFreq) ==
                                    (long) Config.coinFreq) {
                                e.vote[myId] = true;
                            } else {
                                e.vote[myId] =
                                        (event.getSignature()[event.getSignature().length / 2] &
                                                1) == 1;
                            }
                        }
                        ...
                        this.consSetFamous(
                                e.event, this.rounds.get(event.getShardId()).get(e.event.roundCreated),
                                e.vote[myId]);
                    }
                }
            }
    ...
    private RoundInfo consGetOrCreateRoundInfo(int shardId, long round) {
        ...
        if (roundInfo != null) {
            ...
            //}
            this.minRounds.get(shardId).set(this.minRounds.get(shardId).get() == -1L
                    ? round : Math.min(this.minRounds.get(shardId).get(), round) );
            ...
            if (oldRoundInfo != null) {
                ...
                for (RoundInfo.ElectionRound e = oldRoundInfo.elections; e != null;
                     e = e.nextElection) {
    ...
    private RoundInfo.ElectionRound consNewElection(Event witness, RoundInfo roundInfo,
        ...
        RoundInfo.ElectionRound election =
                new RoundInfo.ElectionRound(roundInfo, this.numNodes, witness,
    ...
    private void consSetRoundFameDecidedTrue(int shardId, RoundInfo roundInfo) {
        ...
        for (long round = roundInfo.round;
             this.fameDecidedBelows.get(shardId).get() == round && roundInfo.fameDecided;
             roundInfo = this.rounds.get(shardId).get(round)) {
    ...
    private void consFindReceivedInRound(int shardId, RoundInfo roundInfo) {
        ...
        Collections.sort(consensus, (Event e1x, Event e2x) -> {
            //首先使用共识时戳进行排序
            int c =
                    e1x.getConsensusTimestamp().compareTo(e2x.getConsensusTimestamp());
            if (c != 0) {
                return c;
            } else {
                //然后是共识Round和签名来排序
                ...
                return c != 0 ? c
                        : Utilities.arrayCompare(e1x.getSignature(),
                        e2x.getSignature(), whitening);
            }
        });
...
//        logger.info("consEventsQueue size: " + consEventsQueue.size());
        //使用同步接口来一次性添加
        Collection<Event> syncEventsQueue =
                Collections.synchronizedCollection(this.consEventsQueues.get(shardId));
    ...
    private void consSetIsConsensusTrue(Event event,
                                        RoundInfo receivedRoundInfo) {
        if (!event.isCleared()) {
            //记录接收的Round，并设置状态
    ...
    private void consSetFamous(Event event, RoundInfo roundInfo, boolean isFamous) {
        ...
        //就是一次双向链表的元素删除操作
        for (RoundInfo.ElectionRound e = event.firstElection; e != null;
             e = e.nextRound) {
...
//        }
        ...
        Event evt = new Event(eb.getShardId(), eb.getCreatorId(), eb.getCreatorSeq(),
                eb.getOtherId(), eb.getOtherSeq(), selfParent, otherParent,
                eb.getTimeCreated(), eb.getSignature(), eb.getGeneration(), eb.getHash(), eb.getTrans());
