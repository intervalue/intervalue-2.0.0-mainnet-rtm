...
//    	 existEventKeys.clear();
    ...
    public EventBody getEventInMem(int shardId, long creatorId, long creatorSeq) {
        ...
        if(creatorSeq > 0) {
            ...
            if(otherParent!=null) {
                ...
                if(otherParent==null) {
                    logger.error("(shardId, creatorId, creatorSeq)=({}, {}, {}) other parent is null",
            ...
            if(selfParent!=null) {
                ...
                if(selfParent==null) {
                    logger.error("(shardId, creatorId, creatorSeq)=({}, {}, {}) self parent is null",
                            shardId, creatorId, creatorSeq);
                }
            }
        }
    ...
    public void addEvent(EventBody eb) {
        ...
        try {
            ...
            if (eb.getTrans() != null){
                logger.info("DSPTVM - eb ({}) with txs is gonna to been put into EventSaveQueue using by eventSaveThread ", JSONObject.toJSONString(eb));
            }
...
//            logger.info("node-({}, {}): next(), index: {}, num: {}, flag: {}", this.shardId, this.selfId, index, num, flag);
        ...
        private void getlastSeqs(){
                ...
                if (null!=seqByte && seqByte.length>0) {
                    ...
                    if (!lastSeqs[j].equals(lastSeq)) {
                        logger.warn("node-({}, {}): ({}, {}) lastSeq diff: db-{}, calcu-{}",
                                this.shardId, this.selfId, this.shardId, j, lastSeqs[j], lastSeq);
                        lastSeqs[j] = lastSeq;
            ...
            logger.warn("node-({}, {}): lastSeqs-{}",
                    this.shardId, this.selfId, JSONObject.toJSONString(lastSeqs));
