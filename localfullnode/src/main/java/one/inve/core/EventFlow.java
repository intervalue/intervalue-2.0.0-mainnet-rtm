...
public class EventFlow {
    ...
    public synchronized EventBody newEvent(int shardId, long creatorId, long otherId, byte[][] trans) {
        ...
        EventBody selfParent = (eb.getCreatorSeq() <= 0)
                ? null : eventStore.getEvent(eb.getShardId(), eb.getCreatorId(), eb.getCreatorSeq() - 1);
        EventBody otherParent = (eb.getOtherId()<0 && eb.getOtherSeq()<0)
                ? null : eventStore.getEvent(eb.getShardId(), eb.getOtherId(), eb.getOtherSeq());
        ...
        eb.setGeneration(1L +
                (Math.max((selfParent == null) ? -1L
                        : selfParent.getGeneration(), (otherParent == null) ? -1L : otherParent.getGeneration())) );
        ...
        eb.setHash(Hash.hash(eb.getShardId(), eb.getCreatorId(), eb.getCreatorSeq(),
                selfParentHash, otherParentHash, eb.getTimeCreated(), trans));
        ...
        try {
            ...
            if (eb.getTrans() != null){
                logger.info("DSPTVM - eb ({}) with txs is gonna to been put into eventFlowQueue using by hashnet ", JSONObject.toJSONString(eb));
            }
...
//            logger.error(">>>>>> failed to instatiate add Event because ("+eb.creatorId+", "+eb.creatorSeq+") already exist");
        ...
        EventBody selfParent =
                this.eventStore.getEvent(eb.getShardId(),eb.getCreatorId(), eb.getCreatorSeq() - 1);
        ...
        if (selfParent == null && eb.getCreatorSeq() - 1L != -1L) {
            logger.error(">>>>>> failed to instatiate add Event because selfParent (id,seq) of ("
                    +eb.getCreatorId()+", "+(eb.getCreatorSeq())+") is missing");
            resultMap.put("result", "selfMissing");
            return resultMap;
        }
        if (otherParent == null && eb.getOtherId() != -1L && eb.getOtherSeq() != -1L) {
            logger.error(">>>>>> failed to instatiate add Event because otherParent (id, seq) of ("
                    +eb.getOtherId()+", "+eb.getOtherSeq()+") is missing ");
        ...
        if (selfParent != null && !eb.getTimeCreated().isAfter(selfParent.getTimeCreated())) {
            logger.error(">>>>>> failed to instatiate add Event because (id, seq) of ("
                    +eb.getCreatorId()+", "+eb.getCreatorSeq()+")  timecreated is before than its parent ");
        ...
        byte[] otherParentHash = otherParent == null ? null : otherParent.getHash();
        //本地数据
        byte[] selfHash = Hash.hash(eb.getShardId(), eb.getCreatorId(), eb.getCreatorSeq(),
                selfParentHash, otherParentHash, eb.getTimeCreated(), eb.getTrans());
        //hash验证是否被篡改
        ...
        if ( !Cryptos.verifySignature(selfHash, eb.getSignature(), publicKey) ) {
            ...
            String timeCreatedStr = eb.getTimeCreated()==null ?
                            null : eb.getTimeCreated().getEpochSecond() + "." + eb.getTimeCreated().getNano();
            String hash1 = DSA.encryptBASE64(Hash.hash(eb.getShardId(),
                    eb.getCreatorId(),
                    eb.getCreatorSeq(),
                    selfParentHash,
                    eb.getOtherHash(),
                    eb.getTimeCreated(),
                    eb.getTrans()));
            String hash2 = DSA.encryptBASE64(Hash.hash(eb.getShardId(),
                    eb.getCreatorId(),
                    eb.getCreatorSeq(),
                    eb.getParentHash(),
                    otherParentHash,
        	...
        	if(Cryptos.verifySignature(eb.getHash(), eb.getSignature(), publicKey)) {
                if(DSA.encryptBASE64(eb.getHash()).equals(hash1)
                    ...
                    logger.error("{}===={}===={}",
                            DSA.encryptBASE64(eb.getHash()),
                            DSA.encryptBASE64(eb.getSignature()),
        ...
        if (newSeq != eb.getCreatorSeq()) {
            logger.error(">>>>>> failed to instatiate add Event because otherParent (id, seq) of ("
                    +eb.getOtherId()+", "+eb.getCreatorSeq()+") invalid seq");
    ...
    public synchronized boolean checkSplitEvent(EventBody eb1, byte[] otherHash1, EventBody eb2,
                                                byte[] otherHash2, byte[] parentHash) {
