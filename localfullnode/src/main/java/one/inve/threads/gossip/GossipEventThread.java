    ...
    public void run() {
        ...
        while (true) {
            if (gossipFlag) {
                ...
                List<Member> members = (type == Config.GOSSIP_IN_SHARD)
                        ? node.inshardNeighborPools
...
//                logger.warn("{}there is no neighbor.", pre);
                ...
                for (int ni = 0; ni < numNeighbors; ni++) {
                    ...
                    if (!prxMap.containsKey(neighbor.address())) {
                        Local2localPrx nprx = RpcConnectionService.buildConnection2localFullNode(node.getCommunicator(), neighbor);
                        if (null != nprx) {
                    ...
                    try {
                        ...
                        if (type == Config.GOSSIP_IN_SHARD) {
                            ...
                            logger.info("call gossipMyMaxSeqList4Consensus({}) remotely with parameters of pubkey={},snapshotVersion={},lastSeqs={}",
                                    ips.toString(),
                                    HnKeyUtils.getString4PublicKey(node.publicKey),
                                    node.getCurrSnapshotVersion().toString(),
                                    JSONObject.toJSONString(seqs));
                ...
                // 处理gossip回来的结果
                String ret = handleEventsCompletableFuture(neighborIdxes, connflag, evtResults, numNeighbors, otherShardId, prxMap, true);

                // 检测时间
                long handleInterval = Duration.between(firstTime, Instant.now()).toMillis();
                if (handleInterval > 100) {
                    logger.info("node-({}, {}) {}: {}, evts = {}, size = {} KB, time = {}sec, seq: {}",
                            node.getShardId(), node.getCreatorId(), pre,
                            ips.toString(), ret.split("_")[0], Long.parseLong(ret.split("_")[1]) >> 10,
                            handleInterval / 1000.0, JSONArray.toJSONString(node.maxSeqs));
                }
    ...
    private String gossip2Local(Member neighbor, CompletableFuture<?> evtResult, Local2localPrx prx, int otherShardId, boolean isNewLocalEvent) {
        ...
        try {
            ...
            //logger.error("gossip2Local {} completableFuture.get() Exception: {}:{}-{} ", pre, neighbor.address().host(),neighbor.address().port(), e);
            logger.error("gossip2Local {} completableFuture.get() Exception: host={},rpcPort={},e={} ",
        ...
        if (gossipObj != null) {
            ...
            if (snapVers.compareTo(node.getCurrSnapshotVersion().add(BigInteger.ONE)) > 0) {
                ...
                if (snapVersionMap.get(new String(gossipObj.snapHash))!=null
                        && snapVersionMap.get(new String(gossipObj.snapHash)).size() > (node.getShardCount() * node.getnValue()) / 3 + 1) {
                    ...
                    CompletableFuture<?> snapResult = prxMap.get(neighbor.address())
                            .gossipMySnapVersion4SnapAsync(HnKeyUtils.getString4PublicKey(node.publicKey),
                    ...
                    try {
                        ...
                        //logger.error("gossip2Local for snapshot {} completableFuture.get() Exception: {}-{} ", pre, neighbor.address().host(), e);
                        logger.error("gossip2Local for snapshot {} completableFuture.get() Exception: host={},rpcPort={},e={} ",
                                pre, neighbor.address().host(),neighbor.metadata().get("rpcPort"), e);
                    ...
                    if (snapObj != null) {
                        ...
                        if (SignUtil.verify(originalSnapshotStr)) {
                            ...
                            try {
                                logger.error("node.getConsMessageVerifyQueue().put(JSONObject.parseObject(snapMessageStr))" + snapMessageStr);
                            ...
                            node.getTreeRootMap().put(snapshotMessage.getSnapVersion(),snapshotMessage.getSnapshotPoint().getMsgHashTreeRoot());
            ...
            if (evtRet.length > Config.DEFAULT_SYNC_EVENT_COUNT) {
                ...
                logger.warn("gossiped event number value({}) exceeds system threshold value({}),which leads to the issue of UST",evtRet.length,Config.DEFAULT_SYNC_EVENT_COUNT);
            }
//            if (null != evtRet && evtRet.length > 0 && evtRet.length <=Config.DEFAULT_SYNC_EVENT_COUNT ) {
            if (null != evtRet && evtRet.length > 0 && evtRet.length <=Config.DEFAULT_SYNC_EVENT_COUNT ) {
                // 将获取到的event添加到hashnet
                flag = addEvent2Hashnet(evtRet, (type == Config.GOSSIP_IN_SHARD)
                        ? node.getShardId() : otherShardId);

                //Francis.Deng 4/2/2019
                logger.info("{}=addEvent2Hashnet({},{}),which impacts the generation of 'newLocalEvent'",
                        Boolean.toString(flag),
                        String.valueOf(node.getShardId()),
                        String.valueOf(otherShardId));
    ...
    public boolean gossip2AppointLocalFullNode(String pubkey) {
        ...
        List<Member> neighbors = (type == Config.GOSSIP_IN_SHARD) ? node.inshardNeighborPools : node.globalNeighborPools;
                        ...
                        do {
                            if (type == Config.GOSSIP_IN_SHARD) {
                                ...
                                evtResults = nprx.gossipMyMaxSeqList4ConsensusAsync(
                                        HnKeyUtils.getString4PublicKey(node.publicKey), "", node.getCurrSnapshotVersion().toString(), null, seqs);
                            } else {
                                shardId = Integer.parseInt(neighbor.metadata().get("shard"));
                                long seqs[] = node.getEventStore().getLastSeqsByShardId(shardId);
                                evtResults = nprx.gossipMyMaxSeqList4SyncAsync(
                                        HnKeyUtils.getString4PublicKey(node.publicKey), "", Integer.parseInt(neighbor.metadata().get("shard")), node.getCurrSnapshotVersion().toString(), null, seqs);
                            }
                            result = Integer.parseInt(gossip2Local(neighbor, evtResults, nprx, shardId, false).split("_")[0]);
                        } while (result == findCount);
                    } catch (Exception e) {
                        logger.error("gossipMyMaxSeqList ConnectionRefusedException "
        ...
        try {
            ...
            for (one.inve.rpc.localfullnode.Event evt : events) {
                if (evt.selfSeq == 0 || evt.selfSeq == node.maxSeqs[shardId][(int) evt.selfId] + 1) {
                    ...
                    logger.warn("======== (shardId, creatorId) = ({}, {}) : max_seq = {}, receive seq = {}",
                            shardId, evt.selfId, node.maxSeqs[shardId][(int) evt.selfId], evt.selfSeq);
                    node.maxSeqs[shardId][(int) evt.selfId] = evt.selfSeq;
                }
                EventBody eb = new EventBody.Builder().shardId(evt.shardId)
                        .creatorId(evt.selfId).creatorSeq(evt.selfSeq)
                        .otherId(evt.otherId).otherSeq(evt.otherSeq)
                        ...
                        .consTimestamp((evt.consensusTimestampSecond < 0)
                ...
                if ("selfMissing".equals(resultMap.get("result"))){
                    ...
                    while(successList.size()<nodes){
                        for (Member member : members) {
                            Local2localPrx nprx = RpcConnectionService.buildConnection2localFullNode(node.getCommunicator(), member);
                            try {
                                one.inve.rpc.localfullnode.AppointEvent appointEvent = nprx.gossip4AppointEvent(pubkey, sig == null ? "" : DSA.encryptBASE64(sig), shardId, evt.selfId, evt.selfSeq - 1);
                                successList.add(member);
                                if (appointEvent != null && appointEvent.event != null && appointEvent.event.hash != null && appointEvent.event.hash.length > 0) {
                        ...
                        for(String hash:eventHashMap.keySet()){
                            if(eventHashMap.get(hash).size()>nodes/3+1){
                                ...
                                myFlow.addEvent2Store( new EventBody.Builder().shardId(event.shardId)
                                        .creatorId(event.selfId).creatorSeq(event.selfSeq)
                                        .otherId(event.otherId).otherSeq(event.otherSeq)
                                        .timeCreated(Instant.ofEpochSecond(event.timeCreatedSecond, event.timeCreatedNano))
                                        .trans((null == event.messages || event.messages.length == 0) ? null : event.messages)
                                        .signature(event.sign).isFamous(event.isFamous).generation(event.generation).hash(event.hash)
                                        .consTimestamp((event.consensusTimestampSecond < 0)
                                                ? null : Instant.ofEpochSecond(event.consensusTimestampSecond, event.consensusTimestampNano))
                                        .build());
                ...
                if("otherMissing".equals(resultMap.get("result"))){
                    ...
                    while(successList.size()<nodes){
                        for (Member member : members) {
                            Local2localPrx nprx = RpcConnectionService.buildConnection2localFullNode(node.getCommunicator(), member);
                            try {
                                one.inve.rpc.localfullnode.AppointEvent appointEvent = nprx.gossip4AppointEvent(pubkey, sig == null ? "" : DSA.encryptBASE64(sig), shardId, evt.otherId, evt.otherSeq);
                                successList.add(member);
                                if (appointEvent != null && appointEvent.event != null && appointEvent.event.hash != null && appointEvent.event.hash.length > 0) {
                                    String hash = DSA.encryptBASE64(appointEvent.event.hash);
                                    if (eventHashMap.get(hash) != null) {
                ...
                if ("hashErr".equals(resultMap.get("result"))) {
                	/*one.inve.rpc.localfullnode.Event event = gossipEvent(neighbor,shardId,evt.otherId,evt.otherSeq);
                	int count = 0;
                	//如果event为空，则继续获取event，继续三次后停止。
                	while(event==null) {
                		if(count>3) {
                			break;
                		}
                		Thread.sleep(100);
                		count++;
                		event = gossipEvent(neighbor,shardId,evt.otherId,evt.otherSeq);
                	}
                    ...
                    if ("otherChild".equals(resultMap.get("splitType"))) {
                        logger.info("find split attack:local event creatorId--" + evt.otherId + "--creatorSeq--" + evt.otherSeq);
                        ...
                        logger.info("find split attack:local event creatorId--" + evt.selfId + "--creatorSeq--" + (evt.selfSeq - 1));
                    ...
                    if (pubKey != null) {
                        ...
                        if ("0".equals(node.getShardId()) && "0".equals(node.getCreatorId())) {
                            ...
                            String Event = JSONObject.toJSONString(((Main) node).getEventStore().getEvent(shardId, creatorId, creatorSeq));
                            prx.gossipReport4splitAsync(pubkey
                                    , sig == null ? "" : DSA.encryptBASE64(sig)
                                    , data
                                    , shardId
                                    , Event);
                            while (gossipAppointEventFlag) {
                                ...
                                for (Address address : prxMap.keySet()) {
                                    CompletableFuture<?> evtResult = prxMap.get(address).gossip4AppointEventAsync(pubkey,
                                            sig == null ? "" : DSA.encryptBASE64(sig), shardId, creatorId, creatorSeq);
                                    one.inve.rpc.localfullnode.Event event = (one.inve.rpc.localfullnode.Event) evtResult.get(30000, TimeUnit.MILLISECONDS);
                                    if (event != null && event.hash != null) {
                                        ...
                                        if (appointEventMap.get(DSA.encryptBASE64(event.hash)) != null) {
                                            ...
                                            if (addressSet.size() >= ((node.getShardCount() * node.getnValue()) / 3) + 1) {
                                                //
                                                gossipAppointEventFlag = false;
                                                gossip4SplitDel(shardId, creatorId, creatorSeq, DSA.encryptBASE64(event.hash));
                                                flag = false;
                                                ...
                                                for (Address noticeAddress : noticePrxMap.keySet()) {
                                                    noticePrxMap.get(noticeAddress).gossip4SplitDelAsync(pubkey, sig == null ? "" : DSA.encryptBASE64(sig), data, shardId, creatorId, creatorSeq, event.hash == null ? "" : DSA.encryptBASE64(event.hash), false);
                                                }
                                                break;
                                            }
                                        } else {
                            ...
                            prxMap.putAll(type == Config.GOSSIP_IN_SHARD
                                    ? ((Main) node).getConsensusThread().getPrxMap() : ((Main) node).getSyncThread().getPrxMap());
                            List<Member> allMembers = type == Config.GOSSIP_IN_SHARD
                                    ? node.inshardNeighborPools : node.globalNeighborPools;
                            Member centerNeighbor = null;
                            PublicKey centerPubkey = node.getEventFlow().getPubKeys()[0][0];
                            for (Member member : allMembers) {
                                if (centerPubkey != null && HnKeyUtils.getString4PublicKey(centerPubkey).equals(member.metadata().get("pubkey"))) {
                            ...
                            if (centerNeighbor != null && prxMap.get(centerNeighbor.address()) != null) {
                                ...
                                while (gossipAppointEventFlag) {
									...
									/*allMembers = node.inshardNeighborPools;
									String myPubkey = DSA.encryptBASE64(node.publicKey);
									for (Member member : allMembers) {
										if (!StringUtils.isEmpty(myPubkey) && myPubkey.equals(member.metadata().get("pubkey"))) {
											neighbor = member;
										}
									}*/
                                    for (Address address : prxMap.keySet()) {
										/*if (neighbor != null && neighbor.address() == address) {
											continue;
										}*/
                                        CompletableFuture<?> evtResult = prxMap.get(address).gossip4AppointEventAsync(pubkey, sig == null ? "" : DSA.encryptBASE64(sig), shardId, creatorId, creatorSeq);
                                        one.inve.rpc.localfullnode.AppointEvent appointEvent = (one.inve.rpc.localfullnode.AppointEvent) evtResult.get(30000, TimeUnit.MILLISECONDS);
                	...
                	/*}else {
                		logger.info("find tamper ! hashErr ip:"+neighbor.address().host()+"--wirte in blackPubKeyList!");
                		logger.info("event creatorId:"+event.selfId+"===creatorSeq:"+event.selfSeq+"======hash:"+DSA.encryptBASE64(event.hash));
                		//GossipLocalFullNodeThread.blackPubKeyList.add(neighbor.metadata().get("pubkey"));
                		//writeBlackInDb(neighbor.metadata().get("pubkey"),2);
                		break;
                	}*/
                }
                if ("signatureErr".equals(resultMap.get("result"))) {
                	/*logger.info("find tamper ! signatureErr ip:"+neighbor.address().host()+"=====shard:"+neighbor.metadata().get("shard")+"===index:"+neighbor.metadata().get("index")+"--wirte in blackPubKeyList!");
        ...
        if (localEvent != null && localEvent.getHash() != null
            ...
            for (int i = 0; i < eventBodys.size(); i++) {
                if (eventBodys.get(i) != null) {
                    SplitResult splitResult = splitDel(shardId, eventBodys.get(i).getCreatorId(), eventBodys.get(i).getCreatorSeq());
                    if (minSeq == null) {
