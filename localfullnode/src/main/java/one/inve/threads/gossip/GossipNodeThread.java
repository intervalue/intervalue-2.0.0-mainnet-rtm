...
public class GossipNodeThread extends Thread {
    ...
    public void run() {
        ...
        //logger.info("gossip,rpc = {},{}",node.nodeParameters.selfGossipAddress.gossipPort,node.nodeParameters.selfGossipAddress.rpcPort);

        ClusterConfig nodeConfig;
        nodeConfig = ClusterConfig.builder()
                .seedMembers(
                        one.inve.transport.Address.create(
                                node.nodeParameters.seedGossipAddress.getPubIP(),
                                node.nodeParameters.seedGossipAddress.getGossipPort()
                        )
                )
                .memberHost(node.nodeParameters.selfGossipAddress.getPubIP())
                .port(node.nodeParameters.selfGossipAddress.getGossipPort())
                ...
                .addMetadata("pubkey", this.pubkey)
                .addMetadata("address", this.node.getWallet().getAddress())
                .build();
                        ...
                        .collect( Collectors.collectingAndThen(
                                Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(o ->
                                        new StringBuilder()
                                                ...
                                                .append("_").append(o.address().port())
    ...
    private void clearExpiredMessageHash(GeneralNode node, long nowTime) {
        ...
        if (count > 0) {
            logger.info("node-({}, {}): MessageHashCache remove {} expired key.",
                    node.getShardId(), node.getCreatorId(), count);
        }
    }
    ...
    private void statisticsAndShowTpsInfo(long index) {
        ...
        if (index%freq==0) {
            ...
            logger.info(statisticInfo.toString(), node.getShardId(), node.getCreatorId(),
                    ...
                    totalConsEventCount,
                    totalMsgCount,
                    Config.DEFAULT_GOSSIP_NODE_INTERVAL*freq/1000.0,
                    newConsEventCount, eps,
                    newMgsCount, tps);
        }
    }
...
//        }
        ...
        return  node.getLocalFullNodes().parallelStream()
                .filter(n -> n.getStatus()== NodeStatus.HAS_SHARDED).anyMatch(p -> p.getPubkey().equals(pubkeyStr));
    }
}
