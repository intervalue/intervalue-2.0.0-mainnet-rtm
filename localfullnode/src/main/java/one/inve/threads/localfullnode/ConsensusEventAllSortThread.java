 ...
 * @date   2018/6/10.
 */
public class ConsensusEventAllSortThread extends Thread {
    private static final Logger logger = LoggerFactory.getLogger(ConsensusEventAllSortThread.class);
    private Main node;

    public ConsensusEventAllSortThread(Main node) {
        this.node = node;
    }
    ...
    public void run() {
        ...
        EventBody[] events = new EventBody[node.getShardCount()];
        EventBody temp = null;
        int smallIndex = 0;
        //获取每个分片对应的阻塞队列放入List中
        for (int i = 0; i < node.getShardCount(); i++) {
            queueList.add(node.getShardSortQueue(i));
        }
        ...
        for (int i = 1; i < node.getShardCount(); i++) {
            try {
        ...
        while (true) {
            ...
            try {
                node.getConsEventHandleQueue().put(temp);
            } catch (Exception e) {
