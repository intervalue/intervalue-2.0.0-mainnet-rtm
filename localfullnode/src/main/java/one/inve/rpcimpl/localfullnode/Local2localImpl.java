...
import one.inve.rpc.localfullnode.*;
...
import one.inve.threads.gossip.HandleSplitReportThread;
...
public class Local2localImpl implements Local2local {
    ...
    public Local2localImpl(GeneralNode node){
        this.node = node;
    ...
    private boolean validate(String pubkey) {
		...
		boolean isValid = node.getLocalFullNodes().parallelStream()
				.filter(n -> n.getStatus()== NodeStatus.HAS_SHARDED).anyMatch(p -> p.getPubkey().equals(pubkey));
    ...
    /**
     * Consensus In the same shard:
     * The rpc server receives a max sequence list of Hashnet from a random node in the same shard
     * 1. obtain my own seqs
     * 2. compare both seqs, and get my events that the receiver does not have.
     * 3. call the sender's rpc: gossipHashnet4Consensus
     ...
     * @param seqs list of keys that index the event in Hashnet. Each keys contains a creatorId and a sequenceId.
    ...
    public synchronized GossipObj gossipMyMaxSeqList4Consensus(String pubkey, String sig, String snapVersion,
    	...
    	if(gossipFlag) {
			...
			//There was an accident that x.xx.202.66:35791(who had higher snapVersion "273")
			//retained lower seqs watermark than other's(who had snapVersion "272" meanwhile),
			//which caused serious gossip problem.

			//Each thing is fine if the gap between requester's snapVersion and responder‘snapVersion is 0 or 1
	        ...
	        if( currSnapshotVersion.equals(new BigInteger(snapVersion))
				...
				List<Event> rpcEvents = getUnknownEvents(seqs, addressInfo)
						.stream().map(eventBody -> new Event(eventBody.getShardId(),
                                                    eventBody.getCreatorId(),
													eventBody.getCreatorSeq(),
													eventBody.getOtherId(),
													eventBody.getOtherSeq(),
													eventBody.getTrans(),
													eventBody.getTimeCreated().getEpochSecond(),
													eventBody.getTimeCreated().getNano(),
													eventBody.getSignature(),
													eventBody.isFamous(),
													eventBody.getHash(),
													eventBody.getGeneration(),
													(null == eventBody.getConsTimestamp())
															? -1 : eventBody.getConsTimestamp().getEpochSecond(),
													(null == eventBody.getConsTimestamp())
															? -1 : eventBody.getConsTimestamp().getNano(),
                                                    eventBody.getOtherHash(),
                                                    eventBody.getParentHash()))
				...
				if (handleInterval > Config.DEFAULT_GOSSIP_EVENT_INTERVAL) {
					logger.warn("----- gossipMyMaxSeqList4Consensus() {} interval: {} ms", addressInfo, handleInterval);
				}
				return gossipObj;

			} else if(currSnapshotVersion.compareTo(new BigInteger(snapVersion))>0){
				/*logger.warn("hash:{}",SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion)==null?
						null:SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion));
				logger.warn("{}",SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion)==null?
						null:SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion).getBytes());*/
	        	return new GossipObj(currSnapshotVersion.toString(),null,
						SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion)==null?
								null:SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion).getBytes());
			}else{
    ...
    /**
     * Consensus In another shard:
     * The rpc server receives a max sequence list of Hashnet from a random node in another shard
     * 1. obtain my own cspList
     * 2. compare both cspLists, and get my "confirmed" events that the receiver does not have.
     * 3. call the sender's rpc: gossipHashnet4Sync
	 * @param pubkey requestor's public key
	 * @param sig signature
     ...
     */
    @Override
    public synchronized GossipObj gossipMyMaxSeqList4Sync(String pubkey, String sig, int otherShardId, String snapVersion,
                                                          String snapHash, long[] seqs, Current current) {
		GossipObj gossipObj = null;
    	...
    	if(gossipFlag) {
			...
			if( currSnapshotVersion.equals(new BigInteger(snapVersion))
				...
				String addressInfo = (null == current.con) ? null : current.con.toString();
				logger.info("requestor's pubkey = {}",pubkey);
				List<Event> rpcEvents = getUnknownEvents(otherShardId, seqs, addressInfo)
						.stream().map(eventBody -> new Event(eventBody.getShardId(),
                                                    eventBody.getCreatorId(),
													eventBody.getCreatorSeq(),
													eventBody.getOtherId(),
				...
				/*SnapshotMessage snapshotMessage = SnapshotDbService.querySnapshotMessageByHash(node.nodeParameters.dbId, snapHash);*/
				return new GossipObj(currSnapshotVersion.toString(),null,
                        SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion)==null?
                        null:SnapshotDbService.querySnapshotMessageHashByVersion(node.nodeParameters.dbId,snapVersion).getBytes());
			}else {
	...
	public SnapObj gossipMySnapVersion4Snap(String pubkey, String sig, String hash, String transCount,
		...
		// 获取交易信息
		List<JSONObject> trans = TransactionDbService
				.queryMissingTransactionsBeforeSnapshotPoint(originalSnapshotStr, new BigInteger(transCount), node.nodeParameters.dbId);

		//　构建结果结构
		SnapObj snapObj = new SnapObj(snapshotStr,
				(null==trans) ? null : JSONArray.toJSONString(trans));
    ...
    private ArrayList<EventBody> getUnknownEvents(int shardId, long[] otherCounts, String connInfo) {
		...
		logger.info("\n{} \ngetUnknownEvents(): requestor's seqs: {}, my seqs: {}, gossip event size = {}",
				...
				JSON.toJSONString(otherCounts), JSON.toJSONString(currMyCounts), diffEvents.size());
...
//issue of UST(unavailable submitted transaction): any submitted transaction could not been retrieved via consensus mechanism.
			...
			logger.info("node-({},{}): cutResultUnknownEvents()... size: {}",
					node.getShardId(), node.getCreatorId(), size);
     ...
     */
	@Override
	public boolean gossipReport4split(String pubkey, String sig,String data,int shardId,String event,Current current) {
				...
				if(eventMap!=null) {
					...
					if(pubkeySet!=null) {
						...
						if(mapCount>(node.getShardCount()*node.getnValue()*2)/3){
							new HandleSplitReportThread(node,shardId,eb.getCreatorId(),eb.getCreatorSeq(),mostKey,false).start();
							splitReportCache.get(shardId).remove(keyPair);
						}
					}else {
	...
	private boolean checkSig(String data,String sig,String pubkey ) {
		...
		try {
			...
			return Cryptos.verifySignature(Base64.getDecoder().decode(data),
	...
	@Override
	public boolean gossip4SplitDel(String pubkey, String sig, String data, int shardId, long creatorId,
								   long creatorSeq,String eventHash, boolean isNeedGossip2Center,
								   Current current) {
		PublicKey centerPubkey = ((Main)this.node).getEventFlow().getPubKeys()[0][0];
		if(centerPubkey!=null
				&& HnKeyUtils.getString4PublicKey(centerPubkey).equals(pubkey)
				&& checkSig(data,sig,pubkey)) {
			PublicKey pubKey = ((Main)this.node).getEventFlow().getPubKeys()[shardId][(int)creatorId];
			...
			if(localEvent!=null && localEvent.getHash()!=null
				...
				long lastSeq = ((Main)this.node).getEventStore().getLastSeq(shardId, creatorId);
				for (long seq=creatorSeq; seq<=lastSeq; seq++) {
					EventBody eb = ((Main)this.node).getEventStore().getEvent(shardId, creatorId, seq);
					if (null!=eb) {
						eventBodys.add(eb);
					}
				}
				...
				for(int i=0;i<minSeq.length;i++) {
					if(minSeq[i]!=-1) {
						...
						((Main)node).getEventStore().setLastSeq(shardId, i, minSeq[i]);
						int selfId = (int)node.getCreatorId();
				...
				for(String key :delKey) {
					...
					((Main)node).getEventStore().delEventInCache(shardId, Long.parseLong(param[0]), Long.parseLong(param[1]));
				}
				//是否需要与中心节点先gossip同步数据
				if(isNeedGossip2Center) {
					((Main)this.node).getConsensusThread().gossip2AppointLocalFullNode(HnKeyUtils.getString4PublicKey(centerPubkey));
					((Main)this.node).getSyncThread().gossip2AppointLocalFullNode(HnKeyUtils.getString4PublicKey(centerPubkey));
				}
				try {
					Thread.sleep(2000);
			...
			if((this.node).getShardId() == shardId){
				if((shardId+"_"+creatorId+"_"+creatorSeq).equals(((Main)this.node).getConsensusThread().gossipAppointEventKey)){
					((Main)this.node).getConsensusThread().gossipAppointEventFlag = false;
				}
			}else{
				if((shardId+"_"+creatorId+"_"+creatorSeq).equals(((Main)this.node).getSyncThread().gossipAppointEventKey)){
	...
	public AppointEvent gossip4AppointEvent(String pubkey, String sig, int shardId, long creatorId, long creatorSeq,
		...
		EventBody eventBody = ((Main)node).getEventStore().getEventInMem(shardId, creatorId, creatorSeq);

		if(eventBody!=null) {
			return new AppointEvent(null,new one.inve.rpc.localfullnode.Event(
						eventBody.getShardId(),
						eventBody.getCreatorId(),
						eventBody.getCreatorSeq(),
						eventBody.getOtherId(),
						eventBody.getOtherSeq(),
						...
						(null == eventBody.getConsTimestamp())
								? -1 : eventBody.getConsTimestamp().getEpochSecond(),
						(null == eventBody.getConsTimestamp())
								? -1 : eventBody.getConsTimestamp().getNano(), eventBody.getOtherHash(),eventBody.getParentHash()));
