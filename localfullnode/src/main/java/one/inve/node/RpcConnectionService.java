...
public class RpcConnectionService {
    ...
    public static Local2localPrx buildConnection2localFullNode(Communicator communicator, Member member) {
        String str2Proxy = String.format("Local2local:default -h %s -p %s",
                member.address().host(), member.metadata().get("rpcPort"));
        ...
        try {
            ...
            logger.error("buildConnection2localFullNode(): local full node {} is not connected.", member.address().host());
        }
    ...
    public static Local2localPrx buildConnection2localFullNode(Communicator communicator, String ip, int port) {
        ...
        try {
            prx = Local2localPrx.checkedCast(communicator.stringToProxy(
                    "Local2local:default -h " + ip + " -p " + port));
        } catch (Exception e) {
            logger.error("", e);
        }
        if (null == prx) {
