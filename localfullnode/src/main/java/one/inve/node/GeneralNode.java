...
//                                        node.getShardId(), node.getCreatorId(), pair0.toString(), pair.toString());
	...
	private void createSnapshotPoint(Main node, EventBody event) throws InterruptedException {
		if (node.getTotalConsEventCount().mod(BigInteger.valueOf(Config.EVENT_NUM_PER_SNAPSHOT))
			...
			node.getSnapshotPointMap().put(node.getCurrSnapshotVersion(),
					new SnapshotPoint.Builder().eventBody(event).msgHashTreeRoot(msgHashTreeRoot)
			...
			logger.info("\n=========== node-({}, {}):  vers: {}, msgHashTreeRoot: {}", node.getShardId(),
					node.getCreatorId(), node.getCurrSnapshotVersion(), msgHashTreeRoot);
	...
	private SnapshotPoint calculateLatestSnapshotPoint(Main node) {
		...
		if (null != node.getSnapshotPointMap()
			...
			lastSnapshotPoint = node.getSnapshotPointMap().get(node.getCurrSnapshotVersion().subtract(BigInteger.ONE));
		}
