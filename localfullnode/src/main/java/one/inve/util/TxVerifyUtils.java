...
public class TxVerifyUtils {
     ...
     * @return 有效无效
     */
    public static boolean verifyMessage(String message, GeneralNode node) {
        ...
        // 判断在缓存中是否不存在
        if(node.getMessageHashCache().get(hash) == null || node.getMessageHashCache().get(hash) <= 0 ) {
            // 放入缓存
            node.getMessageHashCache().put(hash, Instant.now().toEpochMilli()) ;
            // 判断在数据库中是否不存在
            byte[] existHashBytes = new RocksJavaUtil(node.nodeParameters.dbId).get(hash);
            if (null == existHashBytes || existHashBytes.length <= 0) {
                // 写入数据库
                JSONObject msgObj = new JSONObject();
                msgObj.put("isStable", false);
                msgObj.put("msg", message);
                new RocksJavaUtil(node.nodeParameters.dbId).put(hash, msgObj.toJSONString());
                isValid = true;
            } else {
        ...
        isValid = verifyLegalSender(pubkey, node.getShardId(), node.getShardCount());
        if (!isValid) {
            return isValid;
        }
        // 签名验证
     ...
     * @return 是否有效
     */
    public static int verifyMessageWithoutSign(JSONObject o, int shardId, GeneralNode node) {
        // 消息重复性验证
        String hash = o.getString("signature");
        if (node.nodeParameters.multiple == 1) {
            ...
            if (null == existHashBytes || existHashBytes.length <= 0 ) {
                ...
                isValid = true;
                logger.warn("node-({}, {}): msg receive and init in db: {}", node.getShardId(), node.getCreatorId(), hash);
            } else {
                logger.error("node-({}, {}): msg already exists in db: {}", node.getShardId(), node.getCreatorId(), hash);
            }
            if (!isValid) {
        ...
        if (type != MessageType.SNAPSHOT.getIndex()) {
            String pubkey = o.getString("pubkey");
            isValid = verifyLegalSender(pubkey, shardId, node.getShardCount());
            if (!isValid) {
                return 0;
            }
        }
     ...
     * @return 验证结果
     */
    public static boolean verifyCreationMessage(JSONObject o, GeneralNode node) {
        // 消息重复性验证
        String hash = o.getString("signature");
        if (node.nodeParameters.multiple == 1) {
            byte[] existHashBytes = new RocksJavaUtil(node.nodeParameters.dbId).get(hash);
    ...
    private static boolean verifyLegalSender(String pubkey, int shardId, int shardCount) {
        ...
        if (shardId != (c & (shardCount-1))) {
            logger.error("pubkey: {}, shardId: {}, shardCount: {}, need shardId: {}",
                    pubkey, shardId, shardCount, c & (shardCount-1));
...
//        System.out.println(verifyLegalSender("1", "1", 2));
        new RocksJavaUtil("0_0").put("33APDAlMvdnvWybQolUDeaRtns1EfXqdgIKKFTZc+oTOsPcbdXvXZahnnW9ssFoMuffDUnZ/l9Fxv1NqjfE7Ce6g0=", "");
