...
import one.inve.mysql.DataSourceConfig;
import one.inve.mysql.MysqlTable;
import one.inve.mysql.MysqlTableField;
...
public class MysqlHelper {
    ...
    /**
     * 批量执行sql
     * 如果使用PreparedStatement必须使用此方法，因为数据库不是自动提交的，必须commit
     * @param sql
     * @return
     * @throws
     * @throws SQLException
...
//                System.out.println("------db: " + this.getConnection().getCatalog());
            rs = metaData.getTables(this.getConnection().getCatalog(), "root", null, new String[] { "TABLE" });    //, "VIEW"
            while (rs.next()) {
...
//                System.out.println("------table: " + tablename);
                ...
                while (crs.next()) {
                    ...
                    fields.add(new MysqlTableField.Builder()
                            .name(columnname).type(columntype).size(columnLen).nullable(nullable)
                            .build());
                }
