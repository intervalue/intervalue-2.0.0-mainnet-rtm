...
import one.inve.beans.dao.TableInfo;
...
public class NewTableCreate {
	...
	/**
	 * 创建新表
	 *   transactions_msg 1：交易产生的消息，如奖励 2：快照产生的信息 3:合约信息
	 */
	 ...
	 *   Snapshotmessages 表名
	 */
	/*public static void createTableSnapshotmessages(MysqlHelper mysqlHelper) {
		try {
			StringBuilder sql = new StringBuilder("CREATE TABLE IF NOT EXISTS snapshotMessages (");
			sql.append("id bigint(20) NOT NULL AUTO_INCREMENT,PRIMARY KEY (`id`),");
			sql.append("snapVersion bigint(20),");
			sql.append("snapHash varchar(3000),");
			sql.append("snapshotPoint LONGTEXT,");
			sql.append("accounts LONGTEXT,");
			sql.append("pubkey varchar(3000),");
			sql.append("signature TEXT,");
			sql.append("timestamp bigint(20), ");
			sql.append("preHash varchar(3000), ");
		...
		logger.info("Table createTableSnapshotmessages successfully==============================");
	}*/
	...
	/**
	 * 创建新表
	 * tableNamePrefix 表名前缀  如:transactions
	 * tableName 表名 如:transactions_0
	 ...
	 * @return 这组message的最大ID，异常时返回0
	 */
	public BigInteger batchInsert(MysqlHelper mysqlHelper, List<JSONObject> entityList, TableInfo tableInfo, String dbId) {
