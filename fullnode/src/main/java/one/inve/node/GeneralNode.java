...
public class GeneralNode {
    ...
    public RegisterPrx buildRegisterConnection2FullNode(String ip, String port) {
        ...
        try {
            prx = RegisterPrx.checkedCast(this.getCommunicator().stringToProxy(
                    "Register:default -h " + ip + " -p " + port));
        } catch (Exception e) {
            logger.error(e);
        }
        if (null == prx) {
    ...
    public RegisterPrx buildRegisterConnection2FullNode(BaseNode fullNode) {
        ...
        if (null == prx) {
            logger.error(">>>>>> Can not build connnection to full node " + fullNode.getIp() + ":" + fullNode.getRpcPort());
            throw new Error("Can not build connnection to full node.");
        }
        return prx;
    }
    ...
    public RegisterPrx buildRegisterConnection2FullNode() {
        ...
        try {
            prx = RegisterPrx.checkedCast(this.getCommunicator().stringToProxy(
                    "Register:default -h " + this.parameters.seedGossipAddress.pubIP
    ...
    public boolean hasSufficientFullnodes() {
        long size = fullNodeList.values().stream()
